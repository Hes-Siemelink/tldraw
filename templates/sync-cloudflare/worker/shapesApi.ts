...
import { getIndexAbove, sortByIndex, validateIndexKey } from '@tldraw/utils'
...
export async function handleCreateShape(request: IRequest, env: Env): Promise<Response> {
	try {
		...
		const siblingShapes = Object.values(records)
			.filter((record: TLRecord): record is TLShape => record.typeName === 'shape')
			.filter((shape) => shape.parentId === parentId)
			.sort(sortByIndex)

		const fallbackIndex = getIndexAbove(siblingShapes.at(-1)?.index ?? null)

		let index = fallbackIndex
		if (body.index) {
			try {
				validateIndexKey(body.index)
				index = body.index as typeof index
			} catch {
				// ignore invalid indices provided by the caller and fall back to autogenerated index
			}
		}
